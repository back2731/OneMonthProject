...
UltraliskCavern::UltraliskCavern(int _playerNumber, POINT buildXY)
{
	...
	buildStatus.buildingGasPrice = 200;

	buildStatus.buildImage = IMAGEMANAGER->FindImage("UltraliskCavern");
	...
	buildStatus.buildingWireFrame = IMAGEMANAGER->FindImage("UltraliskCavernWirefram");
...
HRESULT UltraliskCavern::Init()
{
	return S_OK;
}

void UltraliskCavern::Release()
{
	SAFE_DELETE(progressBar);
}

void UltraliskCavern::Update()
{	
	// 애니메이션 프레임 업데이트
	PlayAnimation();
	PlayPortraits();
...
void UltraliskCavern::Render(HDC hdc)
{	
	//해당 객체가 클릭 되었을 때
	if (isClick && buildStatus.playerNumber == PLAYER1)
	{
...
void UltraliskCavern::RenderUI(HDC hdc)
{
	...
	if (isClick && buildStatus.playerNumber == PLAYER1)
	{
		...
		sprintf_s(str, "Zerg UltraliskCavern");
