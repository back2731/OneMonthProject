...
Hydralisk::Hydralisk(int _playerNumber, POINT birthXY)
{
	...
	unitStatus.unitAtk = 10;
	unitStatus.unitDef = 0;
	unitStatus.unitBaseAtk = 10;
	...
	abilityImage[SLOT2] = IMAGEMANAGER->FindImage("needleSpines");
...
{	
	...
	if (isClick && unitStatus.playerNumber == PLAYER1)
	{
		...
		sprintf_s(str, "%d", UPGRADEMANAGER->GetMissileAttack());
		...
		{
			abilityDescriptionImage[SLOT2] = IMAGEMANAGER->FindImage("needleSpinesUI");
			abilityDescriptionImage[SLOT2]->Render(hdc, abilityRect[SLOT2].right, abilityRect[SLOT2].top);

			sprintf_s(str, "%d + %d", unitStatus.unitBaseAtk, UPGRADEMANAGER->GetMissileAttack());
