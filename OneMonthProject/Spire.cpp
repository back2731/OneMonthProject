...
Spire::Spire(int _playerNumber, POINT buildXY)
{
	progressBar = new ProgressBar;

	buildStatus.buidKind = SPIRE;

	buildStatus.playerNumber = _playerNumber;

	buildStatus.buildingMaxHp = 600;
	buildStatus.buildingCurrentHp = 600;
	buildStatus.buildingAtk = 0;
	buildStatus.buildingDef = 1;
	buildStatus.buildTime = 120;

	buildStatus.buildingMineralPrice = 200;
	buildStatus.buildingGasPrice = 150;

	buildStatus.buildImage = IMAGEMANAGER->FindImage("Spire");
	...
	buildStatus.buildingWireFrame = IMAGEMANAGER->FindImage("SpireWirefram");
	...
	buildStatus.buildRect = RectMake(buildXY.x, buildXY.y, 64 * 2, 64 * 2);
	buildStatus.buildCollisionRect = RectMake(buildXY.x, buildXY.y + 64, 64 * 2, 64);
...
{	
	...
	if (isClick && buildStatus.playerNumber == PLAYER1)
	{
		...
		progressBar->Render
		(hdc, buildStatus.buildRectX - buildStatus.buildingProgressWidth, buildStatus.buildRect.bottom + 5);
	}

	if (buildStatus.playerNumber == PLAYER1)
	{
		buildStatus.buildImage->FrameRender(hdc, buildStatus.buildRect.left - 60, buildStatus.buildRect.top - 80, buildStatus.frameIndexX, buildStatus.frameIndexY);
	}
