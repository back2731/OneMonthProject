...
CreepColony::CreepColony(int _playerNumber, POINT buildXY)
{
	progressBar = new ProgressBar;

	buildStatus.buidKind = CREEPCOLONY;
...
void CreepColony::Release()
{
	SAFE_DELETE(progressBar);
}

void CreepColony::Update()
{	
	// 애니메이션 프레임 업데이트
	PlayAnimation();
...
void CreepColony::Render(HDC hdc)
{	
	//해당 객체가 클릭 되었을 때
	if (isClick && buildStatus.playerNumber == PLAYER1)
	{
	...
	{
		buildStatus.buildImage->FrameRender(hdc, buildStatus.buildRect.left - 60, buildStatus.buildRect.top, buildStatus.frameIndexX, buildStatus.frameIndexY);
	}
...
void CreepColony::RenderUI(HDC hdc)
{
	if (isClick && buildStatus.playerNumber == PLAYER1)
	{
