...
DefilerMound::DefilerMound(int _playerNumber, POINT buildXY)
{
	...
	buildStatus.buildImage = IMAGEMANAGER->FindImage("DefilerMound");
	...
	buildStatus.buildingWireFrame = IMAGEMANAGER->FindImage("DefilerMoundWirefram");
	...
	buildStatus.buildRect = RectMake(buildXY.x, buildXY.y, 64 * 4, 64 * 2);
	buildStatus.buildCollisionRect = RectMake(buildXY.x, buildXY.y + 64, 64 * 4, 64);
...
HRESULT DefilerMound::Init()
{
	return S_OK;
}

void DefilerMound::Release()
{
	SAFE_DELETE(progressBar);
}

void DefilerMound::Update()
{	
	// 애니메이션 프레임 업데이트
	PlayAnimation();
	PlayPortraits();
...
void DefilerMound::Render(HDC hdc)
{
	//해당 객체가 클릭 되었을 때
	if (isClick && buildStatus.playerNumber == PLAYER1)
	{
	...
	{
		buildStatus.buildImage->FrameRender(hdc, buildStatus.buildRect.left, buildStatus.buildRect.top - 64, buildStatus.frameIndexX, buildStatus.frameIndexY);
	}
...
void DefilerMound::RenderUI(HDC hdc)
{
	...
	if (isClick && buildStatus.playerNumber == PLAYER1)
	{
		...
		sprintf_s(str, "Zerg DefilerMound");
