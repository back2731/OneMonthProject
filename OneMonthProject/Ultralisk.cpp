...
Ultralisk::Ultralisk(int _playerNumber, POINT birthXY)
{
	progressBar = new ProgressBar;
	unitStatus.unitKind = ULTRALISK;
	...
	unitStatus.unitImage = IMAGEMANAGER->FindImage("Ultralisk");
	unitStatus.unitWireFrame = IMAGEMANAGER->FindImage("UltraliskWirefram");
	...
	unitStatus.unitPortraitsImage = IMAGEMANAGER->FindImage("portraitsUltralisk");
...
void Ultralisk::RenderUI(HDC hdc)
{	
	// 슬롯 위치 카메라 반영
	SetCommandRect();
	SetAbilityRect();
...
void Ultralisk::PlayAnimation()
{
	if (unitStatus.unitState == IDLE)
	{
		unitStatus.frameIndexX = 7;
	...
	else if (unitStatus.unitState == MOVE)
	{
		...
		{
			unitStatus.frameCount = 0;
			if (unitStatus.frameIndexX >= unitStatus.unitImage->GetMaxFrameX() - 6)
			{
