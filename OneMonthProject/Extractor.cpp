...
Extractor::Extractor(int _playerNumber, POINT buildXY)
{
	progressBar = new ProgressBar;

	buildStatus.buidKind = EXTRACTOR;
	...
	buildStatus.buildingMineralPrice = 50;
	buildStatus.buildingGasPrice = 0;

	buildStatus.buildImage = IMAGEMANAGER->FindImage("Extractor");
	...
	buildStatus.buildingWireFrame = IMAGEMANAGER->FindImage("ExtractorWirefram");
...
void Extractor::Render(HDC hdc)
{
	...
	{
		buildStatus.buildImage->FrameRender(hdc, buildStatus.buildRect.left, buildStatus.buildRect.top - 128, buildStatus.frameIndexX, buildStatus.frameIndexY);
	}
